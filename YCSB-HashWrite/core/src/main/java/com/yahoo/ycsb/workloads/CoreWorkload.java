...
public class CoreWorkload extends Workload
{
  ...
  public static final String HOTSPOT_OPN_FRACTION_DEFAULT = "0.8";
	...
	public boolean doInsert(DB db, Object threadstate)
	{
		// Hash and compare
		/*
		 * CONSISTENCY PART pgaref & panapan ICS-FORTH Fall 2013
		 */
		int keynum = keysequence.nextInt();
		String dbkey = buildKeyName(keynum);

		ByteBuffer keyz = ByteBuffer.wrap(dbkey.getBytes());
		long hash = hash3_x64_128(keyz, keyz.position(), keyz.remaining(), 0)[0];
		long norm = normalize(hash);
		long from=-9223372036854775808L;
		long to=-3074457345618258400L;
		
		while ( !(norm >= from) && (norm <=to)) {
			keynum = keysequence.nextInt();
			dbkey = buildKeyName(keynum);
			keyz = ByteBuffer.wrap(dbkey.getBytes());
			hash = hash3_x64_128(keyz, keyz.position(), keyz.remaining(), 0)[0];
			norm = normalize(hash);
		}
